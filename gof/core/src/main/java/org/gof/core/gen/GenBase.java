...
public abstract class GenBase {
    ...
    public GenBase(String packageName, String targetDir) {
        ...
        // 使用 FreeMarker 2.3.33 推荐的构造方式
        configuration = new Configuration(Configuration.VERSION_2_3_33);

        try {
            // 修复合并后的模板路径
            String tempDir = userDir + "/" + CORE_PATH + "/src/main/java/" + PackageClass.packageToPath("org.gof.core.gen") + "/templates";
            configuration.setDirectoryForTemplateLoading(new File(tempDir));
            configuration.setDefaultEncoding(pageEncoding);
        } catch (IOException e) {
    ...
    protected void writeFile(String targetFileDir, String fileName, String tempName, Object rootMap) throws Exception {
        ...
        try (Writer out = new OutputStreamWriter(new FileOutputStream(target), pageEncoding)) {
    ...
    public static Set<Class<?>> getPluginGenDesFile(String packageName) {
        ...
        //去掉第一个seam
        if(GenBase.pluginDesFileName.contains("seam")) {
            String temp = GenBase.pluginDesFileName.replaceFirst("seam", "");
            //如果有两个seam 则替换第一个出现的
            if(temp.contains("seam"))
