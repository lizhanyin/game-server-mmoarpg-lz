...
/**
 * 参数传递简单包装对象 当传递参数数量不确定时，可以使用本类。
 * 
 * 设置值：new Param("k1", v1, "k2", v2); new Param().put("k1", v1).put("k2", v2);
 * 
 * 获取值：Human m = param.get("human"); //这里会自动转换类型，无需添加(User)显示强转
 */
public final class Param implements ISerializable {
    ...
    /**
     * 有参构造函数 内容为Key：Val形式 例如：new Param("user", user, "param", param)
     * 
     * @param params
     */
    ...
    /**
     * 返回相应键值返回的value 会自动根据接收值进行类型转换，无需强制转换处理。
     * 
     * @param key
     * @return
     */
