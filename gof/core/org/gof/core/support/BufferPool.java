...
public class BufferPool {
    ...
    private static final ConcurrentLinkedQueue<byte[]> buffers = new ConcurrentLinkedQueue<>();

    private static final AtomicInteger countCreated = new AtomicInteger(0); // 已创建的缓冲包个数
    ...
    public static byte[] allocate() {
        ...
        //     LogCore.core.debug("BufferPool缓冲池分配，当前数量={}， 已创建总数量={}", buffers.size(),
        //     countCreated);
        // }
    ...
    public static void deallocate(byte[] buff) {
        ...
        // 日志
        // if(LogCore.core.isDebugEnabled() && log) {
        //     LogCore.core.debug("BufferPool缓冲池回收，当前数量={}， 已创建总数量={}", buffers.size(),
        //     countCreated);
        // }
    }
}
