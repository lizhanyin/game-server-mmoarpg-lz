...
import org.gof.core.support.function.GofFunction10;
import org.gof.core.support.function.GofFunction11;
import org.gof.core.support.function.GofFunction12;
import org.gof.core.support.function.GofFunction2;
import org.gof.core.support.function.GofFunction3;
import org.gof.core.support.function.GofFunction4;
import org.gof.core.support.function.GofFunction5;
import org.gof.core.support.function.GofFunction6;
import org.gof.core.support.function.GofFunction7;
import org.gof.core.support.function.GofFunction8;
import org.gof.core.support.function.GofFunction9;
...
public abstract class Port implements IThreadCase {
    ...
    // ID分配池
    private final IdAllotPoolBase idPool;
            ...
            try {
                ...
                if (serv == null) {
                    ...
                    switch (call.methodParam.length) {
                        ...
                        case 10 -> ((GofFunction10) f).apply(m[0], m[1], m[2], m[3], m[4], m[5], m[6], m[7], m[8], m[9]);
                        case 11 -> ((GofFunction11) f).apply(m[0], m[1], m[2], m[3], m[4], m[5], m[6], m[7], m[8], m[9], m[10]);
                        case 12 -> ((GofFunction12) f).apply(m[0], m[1], m[2], m[3], m[4], m[5], m[6], m[7], m[8], m[9], m[10], m[11]);
                        default -> { }
                    }
                }
            } catch (Exception e) {
    ...
    private void pulseCallResultsTimeoutClean() {
        ...
        // 删除超时的监听
        for (CallResultBase r : timeoutResult) {
            // 删除监听
