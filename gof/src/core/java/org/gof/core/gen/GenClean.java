...
public class GenClean {
    private final String targetDir;                     // 待删除文件夹
    private final String packageName;                   // 待删除最外层包名
    ...
    public void clean() {
        ...
        for (Class<?> clazz : sources) {
            ...
            if (clazz.isAnnotationPresent(GofGenFile.class)) {
                String filePath = targetDir + PackageClass.packageToPath(clazz.getPackage().getName()) + "/"
