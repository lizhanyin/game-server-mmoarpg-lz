...
public class Distr {
    ...
    static {
        ...
        // 连接服务配置
        PORT_STARTUP_NUM_CONN = Integer.parseInt(propPWRD.getProperty("port.startup.num.conn"));
    }
    ...
    private static void readConfig(Properties prop) {
        ...
        for (Entry<Object, Object> e : prop.entrySet()) {
            ...
            switch (prefix) {
                // Node地址配置
                case PREFIX_NODE_ADDR    -> NODE_ADDR.put(key, pv);
                // Port启动配置
                case PREFIX_PORT_STARTUP -> PORT_STARTWITH.put(key, pv);
                // Service启动配置
                case PREFIX_SERV_STARTUP -> SERV_STARTWITH.put(key, pv);
                default -> {
                }
            }
        }
