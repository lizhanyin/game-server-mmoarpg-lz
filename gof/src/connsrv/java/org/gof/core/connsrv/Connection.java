...
@DistrClass(importClass = { Chunk.class, ConnectionStatus.class })
public class Connection extends Service {
    ...
    public void sendMsg(int msgId, Chunk msgbuf) {
        ...
        f.addListener((ChannelFutureListener) (ChannelFuture arg0) -> {
            conBuf.removeMsg();
            // log.info("conBuf msgId {} conBuf.idBufList.size() {} conBuf.size() {}
            // msgbuf.length {}",
    ...
    private void handleIncoming(byte[] msgbuf) {
        ...
        switch (m_status.status) {
            case ConnectionStatus.STATUS_LOGIN, ConnectionStatus.STATUS_GATE -> {
                CallPoint toPoint = new CallPoint(Distr.NODE_DEFAULT, Distr.PORT_DEFAULT, Distr.SERV_GATE);
                port.callSeam(toPoint, new Object[] { m_id, m_status, msgbuf }, "method",
                        CallSeamMethodKey.ACCOUNT_MSG);
                ...
                // 记录日志
                // if(log.isDebugEnabled()) {
                // log.debug("将客户端消息转发给账号服务：account={}, connId={}, msgId={}", m_status.account,
                // m_id, mid);
                // }
            }
            case ConnectionStatus.STATUS_PLAYING -> {
                ...
                // 记录日志
                // if(log.isDebugEnabled()) {
                // log.debug("将客户端消息转发给游戏服务：account={}, connId={}, msgId={}", m_status.account,
                // m_id, mid);
                // }
            }
            case ConnectionStatus.STATUS_LOSTED -> {
